...
class Model:
    def __init__(self, local_rank=-1):
        ...
        self.flownet = multiscaletype(backbonetype(**backbonecfg), **multiscalecfg)
        self.device()
    ...
    def load_model(self, path, rank=0, suffix=None):
        ...
        state_dict = de_wrapper(torch.load(load_path))
    ...
    def load_pretrained_model(self, path, rank=0, suffix=None):
        ...
        if rank <0:
            self.flownet.load_state_dict(de_wrapper(torch.load(load_path)))
        else:
           self.flownet.load_state_dict(add_wrapper(de_wrapper(torch.load(load_path))))
    ...
    def inference(self, img0, img1, scale=1, scale_list=[4, 2, 1], TTA=False, timestep=0.5):
        ...
        _, _, _, pred = self.flownet( torch.cat((img0, img1), 1), timestep=timestep)

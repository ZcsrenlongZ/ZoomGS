...
from model.EDSC import Network as EDSCModel

from model.emavfi.config import *
...
class Model:
    def __init__(self, local_rank=-1):
        self.flownet = EDSCModel(isMultiple=True)
        self.device()
    ...
    def load_pretrained_model(self, path, rank=0, suffix=None):
        ...
        state_dict = torch.load(load_path, map_location=torch.device("cuda"))['model_state']

        if rank <0:
            self.flownet.load_state_dict(de_wrapper(state_dict))
        else:
           self.flownet.load_state_dict(add_wrapper(state_dict))
    ...
    def inference(self, img0, img1, scale=1, scale_list=[4, 2, 1], TTA=False, timestep=0.5):
        ...
        pred = self.flownet([img0, img1, timestep * torch.ones(1, 1, int(img0.shape[2]/2), int(img0.shape[3]/2)).to(img0.device)])
        return pred

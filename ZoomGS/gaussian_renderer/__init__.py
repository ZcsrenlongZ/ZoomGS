...
def render(viewpoint_camera, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0,
           override_color = None, white_bg = False, info=None):
    """
    Render the scene.
    ...
    confidence = torch.ones((pc.get_xyz.shape[0], 1)).cuda()
    ...
    if info is not None:
        ...
        # dir_pp_normalized = dir_pp/dir_pp.norm(dim=1, keepdim=True)
        # deta_x, deta_r, deta_s, deta_c, deta_o = pc._mlp(shs, rotations, scales, dir_pp_normalized, means3D, opacity, c=info) 
        deta_x, deta_r, deta_s, deta_c, deta_o = pc._mlp(shs, rotations, scales, means3D, opacity, c=info['c'], target=info['target']) 
        means3D = means3D + deta_x
        scales = scales + deta_s
        rotations = rotations + deta_r
        shs = shs + deta_c
        opacity = opacity + deta_o
